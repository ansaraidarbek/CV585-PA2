...
def gradient(img):
    ...
    Gy = partial_y(img)
    G = np.sqrt(Gx**2 + Gy**2)
    theta = np.rad2deg(np.arctan2(Gy, Gx) + np.pi)%360
    ######################################
    #        END OF YOUR CODE            #
    ######################################
...
def non_maximum_suppression(G, theta):
    ...
    for i in range(1, H-1):
        for j in range(1, W-1):
            angle = theta[i, j] % 180
            if angle == 0:
                if check(G[i][j], G[i][j-1], G[i][j+1]): 
                    out[i, j] = G[i, j]
            elif angle == 45:
                if check(G[i][j], G[i-1][j-1], G[i+1][j+1]): 
                    out[i, j] = G[i, j]
            elif angle == 90:
                if check(G[i][j], G[i-1][j], G[i+1][j]): 
                    out[i, j] = G[i, j]
            elif angle == 135:
                if check(G[i][j], G[i-1][j+1], G[i+1][j-1]): 
                    out[i, j] = G[i, j]
...
def link_edges(strong_edges, weak_edges):
    ...
    while len(indices):
        ...
        indices = indices[1:]
        edges[i, j] = True
        for (subI, subJ) in get_neighbors(i, j, H, W):
            if weak_edges[subI, subJ] and not edges[subI, subJ]:
                indices = np.append(indices, [[subI, subJ]], axis=0)
                edges[subI, subJ] = True
    ######################################
    #        END OF YOUR CODE            #
    ######################################
